...
interface UserPosition extends Omit<Listing, "id"> {
  // Reuse Listing fields, omit generic id
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="bg-red-600 text-white p-6 flex flex-col">
          ...
          <p className="text-xs text-black mt-4 flex-grow">
            The BTC Layaway Protocol is a simple, intent-based settlement system that allows users to lock in the future
            purchase of BTCN (Bitcoin-native token on Corn) at a fixed price by putting down partial collateral. It
            provides physically settled, asymmetric exposure to BTC without requiring margin, price oracles, or
        ...
        <Card className="p-6">
          ...
          <p className="text-sm mb-6">Set deposit amount: Amount of USDT buyer must post to activate the contract.</p>
            ...
            <Dialog open={isCreateListingModalOpen} onOpenChange={setIsCreateListingModalOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>Set the terms for your BTC listing.</DialogDescription>
                </DialogHeader>
        ...
        <Card className="p-6">
          ...
          <p className="text-sm mb-4">
            Match listings by depositing USDT you want to buy BTC for and the amount of BTC you want to buy.
          </p>
            ...
            <Dialog open={isMatchListingModalOpen} onOpenChange={setIsMatchListingModalOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>Enter the details to match an existing BTC listing.</DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input
                      id="deposit-amount-match"
                      type="number"
                      placeholder="Confirm deposit amount"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {userPositions.map(pos => (
                <Card key={pos.positionId} className="p-3 text-sm bg-white border">
                  ...
                  <div className="text-xs text-gray-700 space-y-0.5 mb-2">
                    <div>
                      <span className="font-medium">Amount:</span> {pos.btcAmount} BTC
                    </div>
                    <div>
                      <span className="font-medium">Strike:</span> ${pos.strikePrice.toLocaleString()}
                    </div>
                    {/* Show deposit differently based on role */}
                    {pos.userRole === "Buyer" ? (
                      <div>
                        <span className="font-medium">Your Deposit:</span> ${pos.deposit.toLocaleString()}
                      </div>
                    ) : (
                      <div>
                        <span className="font-medium">Buyer&apos;s Deposit:</span> ${pos.deposit.toLocaleString()}
                      </div>
                    )}
                    <div>
                      <span className="font-medium">Expiry:</span> {pos.expiry}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    {pos.status === "Active" && pos.userRole === "Poster" && (
                      <Button size="sm" variant="outline" disabled className="flex-1 h-7 text-gray-400">
                        Waiting
                      </Button>
                    )}
        ...
        <Card className="p-6 col-span-1 md:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div>
              <Label htmlFor="filter-price-min" className="text-sm">
                Min Price
              </Label>
              <Input
                id="filter-price-min"
            ...
            <div>
              <Label htmlFor="filter-price-max" className="text-sm">
                Max Price
              </Label>
              <Input
                id="filter-price-max"
            ...
            <div>
              <Label htmlFor="filter-deposit-min" className="text-sm">
                Min Deposit
              </Label>
              <Input
                ...
                type="number"
                placeholder="e.g., 500"
                value={filterDepositMin}
                onChange={e => setFilterDepositMin(e.target.value)}
                className="mt-1"
              />
            </div>
            <div>
              <Label htmlFor="filter-expiry-max" className="text-sm">
                Max Expiry
              </Label>
              <Input
                id="filter-expiry-max"
                type="date"
                value={filterExpiryMax}
                onChange={e => setFilterExpiryMax(e.target.value)}
                className="mt-1"
              />
            </div>
          </div>
              ...
              filteredListings.map(listing => (
                <Card key={listing.id} className="p-3 text-sm bg-gray-50">
                  ...
                  <div className="text-xs text-gray-600">
                    ...
                    <span>Deposit: ${listing.deposit.toLocaleString()}</span> |<span>Expiry: {listing.expiry}</span>
                  </div>
                  <Button size="sm" className="w-full mt-2 bg-gray-800 hover:bg-gray-900 text-white h-8">
                    Match
                  </Button>
                </Card>
              ))
